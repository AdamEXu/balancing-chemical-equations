#!/bin/sh
# Seeds the leaderboard from a SQL dump mounted at $SEED_SQL (default /seed.sql) on the first boot of a
# fresh volume, so a migrated deployment keeps its existing runs. SEED_REPLACE=1 forces a
# reseed, keeping the previous database as a timestamped backup beside it.
set -e
db="${DATA_DIR:-/data}/speedrun.db"
seed="${SEED_SQL:-/seed.sql}"
if [ -s "$seed" ]; then
  echo "seed $seed has $(grep -c "^INSERT" "$seed") rows"
  if [ -f "$db" ] && [ "${SEED_REPLACE:-}" = "1" ]; then
    backup="$db.bak-$(date +%Y%m%d%H%M%S)"
    mv "$db" "$backup"
    echo "SEED_REPLACE=1: moved existing database to $backup"
  fi
  if [ ! -f "$db" ]; then
    python3 -c "import sqlite3,sys; c=sqlite3.connect(sys.argv[1]); c.executescript(open(sys.argv[2]).read()); c.commit()" "$db" "$seed"
    echo "seeded $db from $seed"
  fi
fi
exec "$@"
